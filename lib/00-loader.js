...
(function(risa){
    ...
    risa.Loader = new class {
        constructor() {
            ...
            (() => {
                ...
                if (backgroundCandidates.length > 0) {
                    const background = backgroundCandidates[Math.floor(Math.random() * backgroundCandidates.length)];
                    loader.style.backgroundSize = "cover";
                    loader.style.backgroundImage = "url(" + background + ")";
        ...
        load_css(name) {
            ...
            document.getElementsByTagName("head")[0].appendChild(cache);
        }
    }();
    ...
    (function(){
        ...
        for (var i = 0; i < elts.length; ++i) {
            const elt = elts[i];
            if (elt.hasAttribute("hidden") || elt.classList.contains("hidden") || elt.classList.contains("preload"))
                continue ;
            risa.Loader.load_image(elt.src);
        }
    })();

    /* Find all prelode objects and turn them over to the Loader. */
    (function(){
        const elts = document.getElementsByClassName("preload");
        for (var i = 0; i < elts.length; ++i) {
            const elt = elts[i];
            if (elt.tagName.toLowerCase() === "img") {
                risa.Loader.load_image(elt.src);
                risa.effects.remove(elt);
            }
        }
    })();
    ...
    risa.Loader.load_script("11-effects.js");
